...
1. Find the file with the text you want to fix. Everything is in the `UI/` and `Dialog/` folders, one file per building/toolbar/menu category. If you're not sure which file, use GitHub's search (press `/`) for a snippet of the English or German text.
...
3. Find the line you want to change. Each entry looks like this:
   ```
   { String _name = "SomeInternalName"; String _text = "German text goes here"; }
   ```
...
**Only these German characters are supported**, because the font only has glyphs for them:
```
Ä ä Ö ö Ü ü ß
```
